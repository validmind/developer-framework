...
class LJungBox(Metric):
    ...
    **Signs of High Risk**: A high risk or failure in the model's performance relating to this test might be indicated
    by high Ljung-Box statistic values or low p-values. These outcomes suggest the presence of significant
    autocorrelations in the respective features, which, if not properly considered or handled in the machine learning
    model, can negatively affect model performance or bias.

    **Strengths**: The Ljung-Box test is a powerful tool for detecting autocorrelations within datasets, especially in
    time series data. It provides quantitative measures (statistic and p-value) that allow for precise evaluation of
    autocorrelation and can be instrumental in avoiding issues related to autoregressive residuals and other challenges
    in regression models.

    **Limitations**: Despite its utility, the Ljung-Box test is not without its limitations. For instance, it cannot
    detect all types of non-linearity or complex interrelationships among variables. Additionally, testing individual
    features may not fully encapsulate the dynamics of the data if features interact with each other. Lastly, it is
