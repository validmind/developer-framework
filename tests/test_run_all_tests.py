...
def _setup_tabular_test_inputs():
    ...
    # Usage:
    #
    # For 1 dataset tests or model/datset tests, use the training dataset
    # For 2 dataset tests, use both the training and test datasets
...
def create_unit_test_func(vm_test_id, vm_test_class):
    def unit_test_func(self):
        ...
        if vm_test_class.required_inputs != ["dataset"]:
            logger.debug(
                "Skipping test - only dataset tests supported at the moment %s",
                vm_test_id,
            )
