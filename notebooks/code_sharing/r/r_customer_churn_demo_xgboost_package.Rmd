...
# Every result
display_res <- list()
for (index in 1:length(processed_results)) {
  result <- processed_results[[index]]

  #pretty_model_results <- pretty_print_tables(result$result_tables)

 # lapply(pretty_model_results, print_table)

  # Handle plotly images
  if (length(result$plotly_images) > 0) {
    # l <- htmltools::tagList()
    # for (i in 1:length(result$plotly_images)) {
    #   l[[i]] <- as_widget(result$plotly_images[[i]])
    # }
    # print(l)
    for (i in length(result$plotly_images)) {
      print(result$plotly_images[[i]])
    }
  }

  #if (length(result$matplotlib_images) > 0) {
  #    knitr::include_graphics(unlist(result$matplotlib_images))
  #}
}
```
...
p2 <- plot_ly(data = mtcars, x = ~hp, y = ~mpg, type = 'scatter', mode = 'markers')
p3 <- plot_ly(data = mtcars, x = ~qsec, y = ~mpg, type = 'scatter', mode = 'markers')
