...
vm_r <- vm(
  ...
  api_secret = "...",
  project = "...",
  python_version = "~/.pyenv/versions/3.11.10/bin/python"
)
```
...
register_result <- register_test(
    confusion_matrix,
    test_id = "my_custom_tests.ConfusionMatrix2",
    description = "Confusion matrix metric",
    required_inputs = c("dataset", "model")
)

# Import the run_test function
run_test <- import("validmind.tests")$run_test

# Run your custom test
result <- run_test(
    "my_custom_tests.ConfusionMatrix2",
    inputs = list(model = vm_model, dataset = vm_test_ds),
    show = FALSE
)
```
