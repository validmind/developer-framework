...
child_path <- tempfile(fileext = ".Rmd")
```
...
```{r}
x <- summarize_results(tabular_suite_results)
```

```{r generate-chunks, echo=FALSE}
library(kableExtra)
library(glue)
...
for (result in x) {
  for (res in result$results) {
      ...
      if (!is.null(test) && nrow(test) > 0) {
        ...
        save(test, file = temp_file)

        res_title <- res$metadata$title

        res_file <- tempfile(fileext = ".RData")
        save(res_title, file = res_file)

        my_chunks[[length(my_chunks) + 1]] <- glue("
        ```{{r chunk-{i}, echo=FALSE}}
        load('{temp_file}')
        load('{res_file}')
        ...
        test %>%
          kable() %>%
          kable_styling() %>%
          row_spec(0:nrow(test), color = 'black')
        ```
        ")
...
writeLines(c("---", "title: 'ValidMind Results'", "output: html_document", "---", unlist(my_chunks)), child_path)

# Render the temporary R Markdown file
# output_file <- render(temp_file, output_format = "html_document", quiet = TRUE, output_dir = tempdir())
```
